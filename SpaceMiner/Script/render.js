...
// Function to initialize arrowElements and attach event listeners
function initializeArrowElements() {
    arrowElements = {
    ...
    // Check if all arrow elements exist
    if (!arrowElements.up || !arrowElements.down || !arrowElements.left || !arrowElements.right) {
        console.error("Arrow elements are missing in the DOM.");
        return false; // Return false if elements are missing
    }

    // Attach event listeners to arrow elements
    arrowElements.up.addEventListener('pointerdown', () => { arrowClick('ArrowUp'); });
    arrowElements.up.addEventListener('pointerup', () => { arrowRelease('ArrowUp'); });

    arrowElements.down.addEventListener('pointerdown', () => { arrowClick('ArrowDown'); });
    arrowElements.down.addEventListener('pointerup', () => { arrowRelease('ArrowDown'); });

    arrowElements.left.addEventListener('pointerdown', () => { arrowClick('ArrowLeft'); });
    arrowElements.left.addEventListener('pointerup', () => { arrowRelease('ArrowLeft'); });

    arrowElements.right.addEventListener('pointerdown', () => { arrowClick('ArrowRight'); });
    arrowElements.right.addEventListener('pointerup', () => { arrowRelease('ArrowRight'); });

    console.log("Arrow elements initialized successfully.");
...
// Wait for DOM to load and initialize arrow elements
document.addEventListener('DOMContentLoaded', () => {
    if (!initializeArrowElements()) {
        // Retry initialization every 500ms if elements are missing
        const retryInterval = setInterval(() => {
            if (initializeArrowElements()) {
                clearInterval(retryInterval); // Stop retrying once initialized
            }
        }, 500);
    }
});
